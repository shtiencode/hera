...
func TestEvictSource(t *testing.T) {
	...
	//Send 6 insert queries, same sqlhash but different bind KV, source from ccg so it can be ignored.
	testutil.GetClientInfo().Appname = "userapp1"
	testutil.GetClientInfo().Host = "ccg99userapp1"
...
//	This should trigger bind eviction, bind_eviction_threshold_pct(20) is 2 workers. 3 should work
	...
	testutil.GetClientInfo().Host = "dcg333userapp2"
        for i := 0; i < 6; i++ {
                time.Sleep(200 * time.Millisecond);
		tmp_id := id + i%6 // make each id repeat twice
		if i > 2 {
			testutil.GetClientInfo().Appname = "userapp3"
			testutil.GetClientInfo().Host = "dcg555userapp3"
		}
...
//	this should trigger at least two heavy usage case 
	...
	cnt := testutil.RegexCount ("decr hash:3378653297 bindName:srcAZApp# val:dcg333&userapp2 allowEveryX:31-6" )
	if ( cnt < 1) {
            t.Fatalf ("Error: expected bind throttle decr - allowEveryX:31-6")
        }
	cnt = testutil.RegexCount ("bind throttle decr hash:3378653297 bindName:srcAZApp# val:dcg333&userapp2 allowEveryX:25-2" )
	if ( cnt < 1) {
